...
library(ggcorrplot)
library(cowplot)
library(ggpubr)         # For using ggarrange
library(viridis)
library(tidyquant)      # For computing moving averages
...
```{r, fig.width = 10, fig.fig.height = 14, fig.cap = "Boxplots displaying distribution of mole fraction data for the indicated 20 month windows for each of methane (top left), nitrous oxide (top right), and sulfur hexafluoride (bottom left). PPB = parts per billion."}
...
## NASA Gas Concentration Time Series and Moving Average Analysis

### Gas concentration time series plots
...
```{r, fig.width = 12, fig.height = 12, fig.cap = "Time Series plots for atmospheric SF6 (top left), CH4 (top right), and N2O (bottom left). Shaded red rectangle indicates the window defined in this analysis as the COVID-19 window."}
#----------------------------Time Series ---------------------------------
...
#plotlys
```
```{r, fig.cap = "Interactive ggplotly time series plots of indicated gas atmospheric concentration from 2003 to 2022."}
sf6_ly
ch4_ly
n2o_ly
```
...
```{r, fig.width = 15, fig.height = 15, fig.cap = "Windowed time series plots of the indicated gas concentration over 20 months periods; the timeframe of each period is described in the subfigure title. Each period runs from December of the start year to July of the end year. For example, the time series plot for the period 2003 - 2005 (top left in all figures) is a plot of monthly average gas concentrations from December of 2003 to July of 2005 to mimic the defined COVID-19 window."}
...
### Gas atmospheric concentration heatmaps
...
```{r, fig.width = 7, fig.height = 7, fig.cap = "Heatmap of monthly gas concentrations of the indicated gas for years 2003 - 2021"}
#---------------Heat Map(s)-----------------------------
######################### SF6 #########################
...
```{r, fig.width = 13, fig.height = 13, fig.cap = "Moving Averages of gas atmospheric concentration from 2003 - 2021 using the smoothed moving average (SMA, red) and double exponential moving average (DEMA, blue) approaches."}
#---------------------Moving Average-------------------
##################### sf6 rate of change ###############
...
So, we can see that each of the gases have their own fluctuations in rate of change over time. SF<sub>6</sub> shows a bit more of an upward trend marked by a few dips, whereas CH4 and N2o tend to bounce around a central value. There weren't any glaring differences among the COVID windows, but SF<sub>6</sub> and N<sub>2</sub>O had somewhat irregular dips around the time of the first reported COVID case.
...
```{r, fig.cap = "Pearson's linear correlation between temperature anomaly data in the AIRS v6, AIRS v7, and GHCN v4 datasets."}
...
```{r, fig.cap = "Heatmap of temperature anomalies of years 2002 - 2021 from the NASA GHCN v4 dataset. Years were row-z-scored based on their monthly average temperature anomaly and hierarchically clustered via the complete-linkage algorithm."}
...
t(apply(temp.data.monthly[[3]], 2, scale)) %>%
    Heatmap(cluster_columns = TRUE,
            ...
            name = "Z-score")
```
Interestingly enough, 2020 seems to be one of the most extreme months in terms of temperature difference relative to the reference, whereas 2021 is more similar by Euclidean distance to the years 2015 to 2018, and is comparatively less extreme than 2020 as indicated by the z-scores. The fact that 2020 is near the extreme likely explains why the linear models with COVID predict higher average gas concentrations by 2050.
...
1. Compute the distance $D$ between each possible pair of clusters $X$ and $Y$, defined as: $D(X, Y) = \max\limits_{x \in X, y \in Y} d(x, y)$, where $d(x, y)$ is typically the Euclidean distance between two members of each cluster. In other words, we are finding the **farthest neighbors** between two clusters
2. Identify which two clusters have the smallest distance between them as defined above
...
### PCA, K-Means Clustering, and Nearest Neighbor Analysis
...
First, we use k-means clustering to obtain clusters of years. K-means clustering is an unsupervised algorithm which attempts to classify a dataset into k distinct groups. The algorithm achieves this via the following iterative method:

1. Define an initial set of k-means $m_1^{(0)}, m_2^{(0)}, ..., m_k^{(0)}$, often initialized randomly. These are our initial cluster **centroids**.
2. Assign each observation to the cluster with the nearest mean/centroid by Euclidean distance
3. Update the means of each cluster (i.e., re-calculate centroids): $m_i^{(t + 1)} = \displaystyle \frac{1}{\lvert S_i^{(t)} \rvert} \sum_{x_j \in S_i^{(t)}} x_j$
...
```{r, fig.cap = "Elbow plot of k-means clustering analysis, showing the within-sum-of-squares as a function of the number of clusters (k) chosen. An optimal choice of k occurs near the asymptote of WSS "}
...
```{r, fig.width = 6, fig.height = 6, fig.cap = "PCA plot of years 2002-2021. Years are colored based on the identified groups used in k-means clustering for $k = 3$. Arrows indicate the direction of the original variables as projected onto $R^2$."}
...
```{r, fig.cap = "Interactive Euclidean distance matrix between each year of the NASA GISTEMP dataset. Red color indicates closer distance, whereas blue hues indicate years are further away."}
temp.ghcn %>%
  get_dist() %>%
  fviz_dist() %>%
  ggplotly()
...
# See stackoverflow.com/questions/56456012/ for caption options
...
Across all plots, it was generally determined that there was no noticable difference between the pre and post covid years. 
Predictive measures, rate of change analyses, and correlation assessments suggested a possible difference, but 
the large time frame made small degrees of changes negligable within the plots. Overall, the data did show a consistent
upward trend in gas concentrations and surface temperature anaomalies across time.


SS performed quality assessment and missing data visualization on imported datasets (Section \@ref(quality-assessment) and Figures 2.1 to 2.4). AS contributed to initial dataset searching, wrangling data and production for Figures 3.1 and 3.2, and created long-term atmospheric gas concentration predictions via linear models. RLP was responsible for project management and outlining, as well as data wrangling and production for Figures 3.3 to 3.8, figure interpretation and conclusion. SK was responsible for data importing, initial wrangling and cleaning, heatmap/correlation/clustering analysis (Sections \@ref(downloading-and-filtering), \@ref(data-wrangling), \@ref(heatmap-and-correlation-analysis); Figures 3.10 to 3.15 and Table 1), report generation, and management of the GitHub Repository.

# Availability of Data and Scripts

In addition to the code chunks present in this report, all scripts for wrangling and data analysis, the data (raw and filtered) itself, and the source code for this report (as well as a selection of plotting notebooks) can be found at our public GitHub repository: [https://github.com/rlpatt/Data_Munging](https://github.com/rlpatt/Data_Munging). Additional information is included as a README file on the main page.
